...
hits a rate limit or server error.  Adds exponential backoff on top of
simple cooldown for more resilient handling.
"""
...
class TokenManager:
    ...
    BASE_COOLDOWN: float = 60.0     # rate-limit base cooldown
    SERVER_COOLDOWN: float = 10.0   # 500/503/529
    CONNECT_COOLDOWN: float = 5.0   # connection errors
    ...
    def create_message(self, **kwargs: Any) -> anthropic.types.Message:
        ...
        while attempts < self._total_keys:
            ...
            try:
                ...
                ks.consecutive_failures = 0  # reset on success
                return response
                ...
                if exc.status_code in (529, 500, 503):
                    ...
                    print(f"Key #{self._current_index + 1} server error "
                          f"({exc.status_code}). Cooldown {backoff:.0f}s. "
                          "Rotating...")
                    self._advance()
                    attempts += 1
                ...
                ks.cooldown_until = time.time() + backoff
                print(f"Key #{self._current_index + 1} connection error. "
                      f"Cooldown {backoff:.0f}s. Rotating...")
                self._advance()
                attempts += 1
    ...
    def create_message_stream(self, **kwargs: Any) -> Any:
        ...
        Usage::
            ...
            with token_manager.create_message_stream(**params) as stream:
    ...
    def _pick_key(self) -> _KeyState | None:
        """Return the next usable key, or None if all are on cooldown."""
        now = time.time()
